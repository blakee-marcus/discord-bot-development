...
RULES = {
    ...
    "DB003": (
        "registration_in_ready",
        Severity.FAIL,
        "Command registration inside ready/on_ready handler",
    ),
...
def check_js_file(path: Path, result: CheckResult) -> None:
    ...
    for i, line in enumerate(lines, 1):
        ...
        if re.search(
            r"(createMessageComponentCollector|createReactionCollector)\s*\(\s*\{", stripped
        ):
            window = "\n".join(lines[i:i+4])
        ...
        pattern = r"interaction\s*\.\s*response\s*\.\s*(sendMessage|reply|editReply)\s*\("
        if re.search(pattern, stripped):
    ...
    for i, line in enumerate(lines):
        if re.search(r"client\s*\.\s*on\s*\(\s*['\"]ready['\"]", line):
            ...
            window = "\n".join(lines[i:i+16])
            reg_pattern = (
                r"(application\.commands|applicationCommands|\.commands)"
                r"\.\s*(set|create|bulkOverwrite)"
            )
            if re.search(reg_pattern, window, re.IGNORECASE):
                result.findings.append(Finding(
                    rule_id="DB003",
                    severity=Severity.FAIL,
...
def check_python_file(path: Path, result: CheckResult) -> None:
    ...
    for i, line in enumerate(lines, 1):
        ...
        bot_prefix_pattern = r"(commands\.)?Bot\s*\(\s*command_prefix"
        if re.search(bot_prefix_pattern, stripped) and "intents" not in stripped:
            result.findings.append(Finding(
                rule_id="DB012",
                severity=Severity.WARN,
